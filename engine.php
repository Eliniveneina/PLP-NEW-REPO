...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$unit = $_POST['unit'];
	$miniquantity = $_POST['miniquantity'];
	$bPrice = $_POST['bPrice'];
	$sPrice = $_POST['sPrice'];
...
$count1 = $row1['COUNT'];
	...
	$query2 = "INSERT INTO $table_name1(ItemName, Quantity, Unit, Mini_Quantity, BuyingPrice, SellingPrice, Date)
	 			VALUES('$item', '$quantity', '$unit', '$miniquantity', '$bPrice', '$sPrice', DATE(NOW()))";
...
if($item == "" || $quantity == "" || $unit == "" || $bPrice == "" || $sPrice == "")
	...
	$query2 = "UPDATE $table_name1 
				SET ItemName = '$item', Quantity = '$quantity', Unit = '$unit', Mini_Quantity = '$miniquantity', 
	...
	$invo_memo = $_POST['invo_memo'];
	
	//Retrieve the Quantity of  of Items to be sold
	$query6 = "SELECT Quantity FROM $table_name1 WHERE itemID = '$item'";
	$result6 = mysql_query($query6);
	$row6 = mysql_fetch_array($result6);
		$inner_quant = $row6['Quantity'];
		
if($quantity > $inner_quant)
	{
	include('default0.php');
	echo "<script>alert('Sorry!; The Transaction can not be done. There is only " . $inner_quant ." Items in Stock.')</script>";	
	}
else
	{
...
if($price == "")
	...
	$result1 = mysql_query($query1);
		...
		$result1 = mysql_query($query1);			
				...
				{
		//Create and execute the query to record transaction in the table
		$query1 = "INSERT INTO $table_name2(ItemID, Quantity, Price, Verification_Method, Invo_Memo_No, staffID, DateTime)
					VALUES('$item', '$quantity', '$tPrice', '$verification', '$invo_memo', '" . $_SESSION['staffID'] . "',
							NOW())";
		$result1 = mysql_query($query1);				
				}
		}
	...
	$result1 = mysql_query($query1);
	...
	else
		{
			if($invo_memo == "")
				{
		//Create and execute the query to Update transaction record from the table
		$query1 = "Update $table_name1 SET itemID = '$item', Quantity = '$quantity', Price = '$tPrice' 
					WHERE transID = '$transID'";
		$result1 = mysql_query($query1);			
				}
			else
				{
		//Create and execute the query to Update transaction record from the table
		$query1 = "Update $table_name1 
					SET itemID = '$item', Quantity = '$quantity', Price = '$tPrice', 
						Verification_Method = '$verification', Invo_Memo_No = '$invo_memo' 
					WHERE transID = '$transID'";
		$result1 = mysql_query($query1);			
				}
		}
